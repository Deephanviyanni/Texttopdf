...
const pageBreakBtn = document.querySelector("#pageBreakBtn");
const wordCount = document.querySelector("#wordCount");
const alignLeftBtn = document.querySelector("#alignLeft");
const alignCenterBtn = document.querySelector("#alignCenter");
const alignRightBtn = document.querySelector("#alignRight");
const alignJustifyBtn = document.querySelector("#alignJustify");
const insertBulletsBtn = document.querySelector("#insertBullets");
const insertNumbersBtn = document.querySelector("#insertNumbers");
const lineSpacingSelect = document.querySelector("#lineSpacing");
...
function useTemplate(templateName) {
  ...
  editor.innerHTML = `<div class="paper" contenteditable="true" role="textbox" aria-label="Page 1">${template.html.replace(/>\s+</g, "><").trim()}</div>`;
  ...
  const firstPage = editor.querySelector(".paper");
  ...
  function walk(node) {
    if (node.nodeType === Node.TEXT_NODE) {
      if (node.textContent) {
        ...
        let parsedLineHeight = null;
        if (styles.lineHeight && styles.lineHeight !== "normal") {
          parsedLineHeight = parseFloat(styles.lineHeight);
        }
    ...
    if (node.nodeName !== "DIV" && !node.classList.contains("paper") && ["DIV", "P"].includes(node.nodeName)) {
      runs.push({ text: "\n", color: "#202124", bold: false, font: "helvetica", fontSize: 12 });
    }
  }

  const pages = editor.querySelectorAll(".paper");
  pages.forEach((page, pageIndex) => {
    if (pageIndex > 0) {
      runs.push({ text: "", color: "#202124", bold: false, font: "helvetica", fontSize: 12, pageBreak: true });
    }
...
function createPdfBlob({ title, runs, pageSize }) {
  ...
  lines.forEach((line) => {
    const hasPageBreak = line.some((run) => run.pageBreak);
    const tallest = Math.max(...line.map((run) => run.fontSize), 12);
    
    const customLineHeightRun = line.find((run) => run.lineHeight);
    const lineHeight = customLineHeightRun 
      ? Math.round(customLineHeightRun.lineHeight) 
      : Math.round(tallest * 1.45);

    if (y < margin || hasPageBreak) {
      pages.push([]);
  ...
  pages.forEach((page) => {
    ...
    page.forEach(({ y: lineY, line }) => {
      const totalLineWidth = line.reduce((sum, run) => sum + measureText(run.text, run), 0);
      const align = line[0]?.align || "left";
      let x = margin;

      if (align === "center" || align === "-webkit-center") {
        x = margin + (maxWidth - totalLineWidth) / 2;
      } else if (align === "right" || align === "-webkit-right") {
        x = margin + (maxWidth - totalLineWidth);
      }
  ...
  let pdf = "%PDF-1.4\n";
  let byteLength = encoder.encode(pdf).length;
  const offsets = [0];
  objects.forEach((body, index) => {
    offsets.push(byteLength);
    const objStr = `${index + 1} 0 obj\n${body}\nendobj\n`;
    pdf += objStr;
    byteLength += encoder.encode(objStr).length;
  });
...
function applyList(listCommand) {
  restoreSelection();
...
function applyLineSpacing() {
  ...
  if (block && editor.contains(block) && !block.classList.contains("paper")) {
    block.style.lineHeight = lineSpacingSelect.value;
  } else {
    const page = node.closest(".paper");
    if (page && editor.contains(page)) {
      page.style.lineHeight = lineSpacingSelect.value;
    }
  }
  updatePreview();
}
...
pageSizeInput.addEventListener("change", updatePreview);
textColorInput.addEventListener("input", applyColor);
boldTextButton.addEventListener("click", applyBold);
fontFamilyInput.addEventListener("change", applyFont);
fontSizeInput.addEventListener("input", applySize);
pageBreakBtn.addEventListener("click", insertPageBreak);

alignLeftBtn.addEventListener("click", () => applyAlignment("justifyLeft"));
alignCenterBtn.addEventListener("click", () => applyAlignment("justifyCenter"));
alignRightBtn.addEventListener("click", () => applyAlignment("justifyRight"));
alignJustifyBtn.addEventListener("click", () => applyAlignment("justifyFull"));
insertBulletsBtn.addEventListener("click", () => applyList("insertUnorderedList"));
insertNumbersBtn.addEventListener("click", () => applyList("insertOrderedList"));
...
clearBtn.addEventListener("click", () => {
  editor.innerHTML = `<div class="paper" contenteditable="true" role="textbox" aria-label="Page 1"><br></div>`;
  savedRange = null;
  updatePreview();
